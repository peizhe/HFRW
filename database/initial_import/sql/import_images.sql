...
CREATE TEMPORARY TABLE tmp(id VARCHAR(32), class VARCHAR(32), format VARCHAR(10), width INT, height INT, size INT, content LONGTEXT);
LOAD DATA INFILE 'c:/import_data/images.csv' INTO TABLE tmp
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';

INSERT INTO recognition_data(id, class_code, image_format, image_width, image_height, image_size,
                             image_content, create_date, create_by, edit_date, edit_by)
SELECT t.id, t.class, t.format, t.width, t.height, t.size, t.content, NOW(), 'kolexandr', NOW(), 'kolexandr' FROM tmp t;
